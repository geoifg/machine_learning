...
##Elaborando o modelo XGBoost
library(xgboost)
modelo_xgboost <- train(Class ~ .,  data = treinamento_class, method = "xgbTree",
                   trControl=trainControl(method = "CV", number = 5),
                   metric="Accuracy")
print(modelo_xgboost)

modelo_xgboost$finalModel


##Plot comparando os modelos gerais para os acidentes
comparacao <- resamples(list(C4.5=modelo_c45, C5.0=modelo_c50, RF=modelo_RF, XGBoost=modelo_xgboost))
...
xgboost_pred <- predict(xvars, modelo_xgboost, filename="D:/Documentos/IFG/MACHINE_LEARNING/DADOS/modelo_xgboost_LULC.tif", type="raw", 
                    index=1, na.rm=TRUE, progress="window", overwrite=TRUE)
...
#Em seguida é criado uma matrix (table) entre 
#o dado de validação e o predito pelo modelo
XGBoost_tabela <- table(validacao_class$Class, XGBoostpred)
